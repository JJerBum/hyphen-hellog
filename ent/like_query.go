...
func (lq *LikeQuery) QueryAuthor() *AuthorQuery {
	query := (&AuthorClient{config: lq.config}).Query()
...
func (lq *LikeQuery) Clone() *LikeQuery {
	...
	return &LikeQuery{
		...
		withAuthor: lq.withAuthor.Clone(),
...
func (lq *LikeQuery) WithAuthor(opts ...func(*AuthorQuery)) *LikeQuery {
	query := (&AuthorClient{config: lq.config}).Query()
...
func (lq *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	...
	if lq.withAuthor != nil || lq.withPost != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := lq.withAuthor; query != nil {
		if err := lq.loadAuthor(ctx, query, nodes, nil,
			func(n *Like, e *Author) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
...
func (lq *LikeQuery) loadAuthor(ctx context.Context, query *AuthorQuery, nodes []*Like, init func(*Like), assign func(*Like, *Author)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Like)
	for i := range nodes {
		if nodes[i].author_likes == nil {
			continue
		}
		fk := *nodes[i].author_likes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "author_likes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (lq *LikeQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Like, init func(*Like), assign func(*Like, *Post)) error {
	...
	nodeids := make(map[int][]*Like)
	for i := range nodes {
		if nodes[i].post_likes == nil {
			continue
		}
		fk := *nodes[i].post_likes
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_likes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
